...
2. Setup an SSH key by using one of the two following methods  
   ...
   ```zsh
   curl https://raw.githubusercontent.com/thomasgreen/dotfiles/HEAD/ssh.sh | sh -s "<your-email-address>"
   ```

3. Clone this repo to `~/.dotfiles` with:

    ```zsh
    git clone --recursive git@github.com:thomasgreen/dotfiles.git ~/.dotfiles
    ```
